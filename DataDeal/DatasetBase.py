...
class RealDataSet(Dataset):
    def __init__(self, crop_size: list = None, aug: list = None, n_repeat: int = 1,
        ...
        用于去噪数据集的基类，提供了创建自定义数据集类所需实现的方法和参数。
        ...
        to build custom dataset class, below functions must be implemented in the inherited class. (or see other dataset class already implemented.)
            ...
            - 该方法负责查找数据集中的所有图像，并将它们的路径保存到 self.img_paths 属性中
            - self._load_data(self, data_idx) : load single paired data from idx as a form of dictionary.
            - 该方法加载指定索引的数据，并将其以字典的形式返回，通常包括干净图像和带噪声图像

        Args:
            add_noise (str)     : configuration of additive noise to synthesize noisy image. (see _add_noise() for more details.) 配置添加的噪声类型，用于合成带噪声的图像
            crop_size (list)    : crop size, e.g. [W] or [H, W] and no crop if None 裁剪的大小，例如 [W] 或 [H, W]。如果为 None，则表示不进行裁剪
            aug (list)          : list of data augmentations (see _augmentation() for more details.) 数据增强的列表。使用 _augmentation() 方法进行数据增强处理
            n_repeat (int)      : number of repeat for each data. 每个数据样本的重复次数。这可以用于增加数据集的多样性
            n_data (int)        : number of data to be used. (default: None = all data) 要使用的数据数量。默认为 None，表示使用所有数据
            ratio_data (float)  : ratio of data to be used. (activated when n_data=None, default: None = all data) 要使用的数据比例。当 n_data=None 时激活，默认为 None，表示使用所有数据
        '''
