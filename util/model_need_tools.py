...
def self_ensemble(fn, x):
    '''
    Geomery self-ensemble function 几何自集成（self-ensemble）函数，用于图像去噪
    目标：自集成的主要目标是提高模型的泛化能力，减少结果中的随机性和噪声。这在许多任务中都非常重要，例如图像去噪、图像分类、图像分割等。
    基本思想：通过对同一图像进行多种变换，模型可以从不同的视角提取特征，最终将这些特征进行结合，以期获得更高质量的输出。
    作用：
        - 减少噪声：通过平均多个结果，自集成能有效减少模型输出中的随机噪声。
        - 提高准确性：综合多次推理的结果，可以提高模型的预测准确性，尤其是在输入数据存在一定噪声时。
        - 增强泛化能力：通过不同的视角学习特征，有助于模型在未见过的数据上表现更好，从而提高泛化能力。
